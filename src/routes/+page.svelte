<script lang="ts">
	...
	$: code = JSON.stringify({ messages: $output }, null, 2);

	$: {
		if (allChecked) {
			checked = Array($output.length).fill(true);
		} else {
			allChecked = false; // partial selection
		}
	}
	function deleteCheckedItems() {
		// Initialize an empty array to hold items that are not checked
		let newOutput = [];

		// Loop through the existing $output array
		for (let index = 0; index < $output.length; index++) {
			// If the item at the current index is not checked, add it to newOutput
			if (!checked[index]) {
				newOutput.push($output[index]);
			}
		}

		// Update the Svelte store with the filtered items
		output.set(newOutput);

		// Reset the checked array to match the new length of items, all set to false
	...
	<div class="max-w-5xl w-full h-full relative items-start flex flex-col gap-2">
		{#if tabOpen}
			<div class="flex flex-col items-end w-full gap-2">
				...
				<div class="rounded-lg overflow-hidden h-full w-full relative">
